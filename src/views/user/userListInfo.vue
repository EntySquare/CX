                  ...
                  <template #cell="{ record }">
                    <a-space wrap>
                      ...
                      <a-button
                        type="primary"
                        :status="
                          record.income_state === true ? 'danger' : 'normal'
                        "
                        @click="prohibitClick(1, record)"
                        >禁止收益</a-button
                      >
                      <a-button
                        type="primary"
                        :status="
                          record.withdrawal_state === true ? 'danger' : 'normal'
                        "
                        @click="prohibitClick(2, record)"
                        >禁止提现</a-button
                      >
...
const prohibitClick = (typeNum: number, record: any) => {
  visiblep.value = true;
  walletAddress.value = record;
  type.value = typeNum;
};
const prohibit = async () => {
  ...
  if (type.value === '1') {
    // 禁止提现
    state.value = !walletAddress.value.withdrawal_state;
  } else {
    // 禁止收益
    state.value = !walletAddress.value.income_state;
  }

  const res = await updateUserState({
    wallet_address: walletAddress.value.wallet_address,
    state: state.value,
    type: type.value,
  });
